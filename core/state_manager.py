...
class StateManager:
    ...
    def save_state(self, optimizer_state: Dict[str, Any], 
        ...
        try:
            ...
            if checkpoint_name is None:
                ...
                checkpoint_name = f"checkpoint_iter_{iteration_num:04d}_{timestamp}"
    ...
    def list_checkpoints(self) -> List[Dict[str, Any]]:
        ...
        # 按迭代次数排序（确保iteration是整数类型）
        def safe_iteration_key(checkpoint):
            iteration = checkpoint['iteration']
            if isinstance(iteration, str):
                try:
                    return int(iteration)
                except (ValueError, TypeError):
                    return -1  # 无效的iteration放在最后
            return iteration if isinstance(iteration, int) else -1
        
        checkpoints.sort(key=safe_iteration_key, reverse=True)
        return checkpoints
